...
> **Inspecting Network Requests:** Because dictionary and translation requests run in the background worker to bypass webpage CORS boundaries, their HTTP requests appear in the **background DevTools Network tab** (`chrome://extensions` → "service worker"), not the in-page DevTools.
