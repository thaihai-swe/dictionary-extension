...
import { IconCheck, IconChevronDown, IconCopy, IconMic, IconSpeaker } from '@/components/icons';
import {
  playPronunciation,
  startSpeechPractice,
  useDictionaryAudio,
            ...
            <summary className="flex cursor-pointer list-none items-center justify-between gap-2 text-[12px] font-semibold text-content-secondary outline-none focus-visible:ring-2 focus-visible:ring-accent/40 rounded-lg">
              <span className="flex items-center gap-2">
                ...
                <span className="font-mono text-[11px] text-content-muted" aria-live="polite">
                  {result.sources.filter((source) => source.status === 'contributed').length} contributed
                  {result.sources.some((source) => source.status === 'not_found')
                    ? ` · ${result.sources.filter((source) => source.status === 'not_found').length} no usable result`
                    : ''}
                </span>
              </span>
