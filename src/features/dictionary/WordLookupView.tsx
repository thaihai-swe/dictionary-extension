...
export const WordLookupView: React.FC<WordLookupViewProps> = ({
  ...
  return (
    <div className="font-sans">
      <div className="workbench-search px-3.5 py-2.5 sticky top-0 z-20">
        <div className="relative flex-1 min-w-0 flex items-center group">
          <IconSearch className="w-4 h-4 text-content-muted absolute left-3 pointer-events-none group-focus-within:text-accent transition-colors" />
          <input
          ...
          {isLoading ? (
            <span className="absolute right-3 pointer-events-none flex items-center justify-center">
              <IconSpinner className="w-4 h-4 text-accent animate-spin" />
            </span>
          ) : searchInput ? (
            <button
              type="button"
            ...
            <span className="hidden sm:inline-flex absolute right-3 px-1.5 py-0.5 rounded-md text-[11px] font-mono text-content-muted/70 border border-border/70 bg-surface select-none pointer-events-none shadow-2xs">
              ↵ Enter
            </span>
          )}
        </div>
