...
<template>
  <div v-if="hasData" class="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-3.5 space-y-2">
    ...
    <div v-if="prefixes?.length || suffixes?.length" class="flex flex-wrap gap-1.5">
      <span v-for="item in prefixes" :key="`p-${item}`" class="px-2 py-0.5 rounded bg-dark-muted text-slate-100 text-[11px] font-semibold">{{ item }}</span>
      <span v-for="item in suffixes" :key="`s-${item}`" class="px-2 py-0.5 rounded bg-dark-muted text-slate-100 text-[11px] font-semibold">{{ item }}</span>
    </div>
